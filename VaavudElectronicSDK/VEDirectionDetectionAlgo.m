...
float compensation2[TICKS_PR_REV] = {1.36642200028858,1.14663273821641,-2.17037166831898,-1.25544425914286,-3.09283096100997,1.03853721398508,1.00103346210481,5.38022993669676,-2.62548084741271,1.81677551601541,-3.58838920481080,1.02778809390994,-1.05007481464095,1.76916037125822,-0.763987577138973};

float compensation3[TICKS_PR_REV] = {0.482503210666667,0.257373151333333,0.318363569333333,0.383400920666667,0.469710791333333,0.653712393333333,0.0889313200000000,0.322430538000000,-0.0822921500000000,-0.624701627333333,-0.254397380000000,-0.132323110000000,-0.451682820000000,-0.298732492666667,-0.227766400000000};


//float compensationT1[TICKS_PR_REV] = {0.5673,0.3535,0.2140,-0.4933,-0.9837,-0.9791,-0.3582,-0.0840,0.5141,1.1365,1.1198,-0.1993,-0.0819,-0.0614,-0.6642};

float compensationT1[TICKS_PR_REV] = {0.255313870261152,-1.06321284790957,-1.85809500396847,-0.485670500175613,0.304926712381872,0.731881867917250,1.45702351021075,1.30215137451077,1.21273480907476,0.686947775212225,0.0957051710857151,-0.902816536905571,-0.308586035140004,-0.288909983469392,-1.13939418308586};
float compentationT1Ispo[TICKS_PR_REV] = {0.326978570833333,-0.299902298333333,-0.676498258333333,-1.62413215833333,-1.62867459833333,-1.56009190000000,-0.425381556666667,0.630586347500000,0.639615490000000,0.855704600000000,0.754921124166667,0.123426450000000,0.943220616666667,1.34049741666667,0.454642833333333};

float compensationS12[TICKS_PR_REV] = {2.45987670000000,0.800626565000000,0.588490516666667,-0.935374225000000,-0.560158465833333,-0.328313783333333,0.122647412500000,0.264504658333333,-0.127888691666667,-0.258119426666667,-0.158305491666667,-1.92416610000000,0.0561986100000000,-0.175370266666667,0.132928903250000};
...
//        compensation[i] = compensation[i]*(compensation2[i]/100+1)*(compensation3[i]/100+1);
...
- (void) locateStart:(int)samples{
    if (samples > 1.2 * lastSample && samples < 1.4 * lastSample) {
        ...
        NSLog(@"Trying to locate start: Ratio: %f, StartCounter: %d", samples / ((float) lastSample), startCounter);
...
- (void) newTick:(int)samples {
    ...
    if (samples > 44100) {
        [self resetDirection];
        NSLog(@"reset: Samplet over 44100");
    }
    if (!startLocated) {
    ...
    if (tickCounter == TICKS_PR_REV-1) {
        ...
        if (samples < 0.9 * lastSample || samples > 1.7 * lastSample) {
        ...
        if (samples < 0.6 * lastSample || samples > 1.3 * lastSample) {
            NSLog(@"Standard Tick Out of ratio: %f", samples / ((float) lastSample));
            [self resetDirection];
            return;
        }
...
- (void) updateUI {
    ...
    for (int i = 0; i < TICKS_PR_REV; i++) {
        ...
        mvgRelativeSpeed[i] = (mvgRelativeTimeUse * compensation[i] -1) * (-100) - compensationS12[i]; // - compentationT1Ispo[i];
    }
...
- (void) iterateAngle: (float *) mvgRelativeSpeedPercent {
    ...
    for (int i = 1; i < TICKS_PR_REV-1; i++) {  //NOW uses all points //starts at 1 and ends before last tick, to avoid errors based on these two.
